	...
	public class IKSolverAimInspector: IKSolverInspector {
		...
		public static void AddScene(IKSolverAim solver, Color color, bool modifiable) {
			...
			for (int i = 0; i < solver.bones.Length; i++) {
				...
				Handles.SphereHandleCap(0, solver.bones[i].transform.position, Quaternion.identity, jointSize, EventType.Repaint);
			}
			
			if (solver.axis != Vector3.zero) Handles.ConeHandleCap(0, solver.transform.position, Quaternion.LookRotation(solver.transform.rotation * solver.axis), jointSize * 2f, EventType.Repaint);
			
			// Selecting joint and manipulating IKPosition
			if (Application.isPlaying && solver.IKPositionWeight > 0) {
				if (modifiable) {
					Handles.SphereHandleCap(0, solver.IKPosition, Quaternion.identity, selectedSize, EventType.Repaint);
