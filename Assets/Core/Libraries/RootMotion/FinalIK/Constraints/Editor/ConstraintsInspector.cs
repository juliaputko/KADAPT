...
namespace RootMotion.FinalIK {
	...
	public class ConstraintsInspector: IKSolverInspector {
		...
		public static void AddScene(Constraints constraints, Color color, bool modifiable) {
			...
			// Transform
			Handles.SphereHandleCap(0, constraints.transform.position, Quaternion.identity, jointSize, EventType.Repaint);
			...
			if (Application.isPlaying && modifiable && (constraints.positionConstraint.weight > 0 || constraints.rotationConstraint.weight > 0)) {
				Handles.CubeHandleCap(0, constraints.positionConstraint.position, constraints.rotationConstraint.rotation, selectedSize, EventType.Repaint);
